...
public class Cantidad extends JFrame {
				...
				if (incremento >= 30) {
					...
					if (incremento != 0) {
						...
						if (!existeProducto(Fr_Comidas.idBoton, 1)) {
							...
							int importeCelda = Integer
									.parseInt(JP_Display.grillaProductos.getValueAt(filaExiste, 3).toString());
	...
	protected void estableceIcono(JButton btn, String url, int x, int y, int width, int height) {
		...
		ImageIcon icono = new ImageIcon(
				icon.getImage().getScaledInstance(btn.getWidth(), btn.getHeight(), Image.SCALE_DEFAULT));
		btn.setIcon(icono);
