...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 10-04-2022 a las 06:58:13
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `contar_registros_comanda` (IN `id9` VARCHAR(100), IN `plato99` VARCHAR(100))  SELECT SUM(cantidad) as 'contar' FROM comanda WHERE id_comanda = id9 AND  plato = plato99$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminar_datos_comanda_edicion` (IN `id2` VARCHAR(100))  DELETE FROM comanda_edicion where id = id2$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listar_comanda` (IN `id_comanda1` VARCHAR(100))  SELECT plato, cantidad, precio_unitario, importe
FROM comanda
WHERE id_comanda = id_comanda1$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `listar_comanda_edicion` (IN `id4` VARCHAR(100))  SELECT *
FROM comanda_edicion
WHERE id = id4$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_restablece_mesa` ()  UPDATE mesa
SET estado = 'Disponible'$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_vaciar_cap_Dato` ()  DELETE FROM cap_datos$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_vaciar_Carga_Boleta` ()  DELETE FROM carga_boleta$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_vaciar_Comanda` ()  DELETE FROM comanda$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_vaciar_Comanda_Edicion` ()  DELETE FROM comanda_edicion$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_verificar_contenido` (IN `id9` VARCHAR(100), IN `plato99` VARCHAR(100))  SELECT COUNT(plato) as 'contar' FROM comanda WHERE id_comanda = id9 AND  plato = plato99$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_verificar_pass` (IN `pass` VARCHAR(100))  select * from admin WHERE password = pass$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sumar_total_comanda` (IN `id` VARCHAR(100))  SELECT SUM(importe) as total FROM comanda
WHERE id_comanda = id$$
...
--
-- Estructura de tabla para la tabla `comanda_edicion`
--

CREATE TABLE `comanda_edicion` (
...
--
-- Indices de la tabla `comanda_edicion`
--
ALTER TABLE `comanda_edicion`
  ADD KEY `fk_mesa` (`fk_mesa`),
  ADD KEY `fk_comida` (`fk_comida`),
  ADD KEY `id` (`id`);
...
--
ALTER TABLE `carga_boleta`
  MODIFY `id_carga` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=73;

--
-- AUTO_INCREMENT de la tabla `comanda`
--
ALTER TABLE `comanda`
  MODIFY `id_unico` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=298;
...
--
-- Filtros para la tabla `comanda_edicion`
--
ALTER TABLE `comanda_edicion`
  ADD CONSTRAINT `comanda_edicion_ibfk_1` FOREIGN KEY (`fk_mesa`) REFERENCES `mesa` (`id_mesa`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `comanda_edicion_ibfk_2` FOREIGN KEY (`fk_comida`) REFERENCES `comida` (`id_comida`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `comanda_edicion_ibfk_3` FOREIGN KEY (`id`) REFERENCES `comanda` (`id_comanda`) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- Eventos
--
CREATE DEFINER=`root`@`localhost` EVENT `vacia_tablas` ON SCHEDULE EVERY 1 MINUTE STARTS '2022-03-26 23:55:22' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    DELETE FROM comanda WHERE UNIX_TIMESTAMP(fecha_hora) < (UNIX_TIMESTAMP()-1440*3600);
    DELETE FROM pre_boleta WHERE UNIX_TIMESTAMP(fecha_hora) < (UNIX_TIMESTAMP()-1440*3600);
	END$$
